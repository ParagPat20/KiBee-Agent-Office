...
const S: Record<string, React.CSSProperties> = {
  // Header
  headerTitle: { fontFamily: F, fontSize: 15, fontWeight: 700, color: 'var(--color-accent-bright)', letterSpacing: '0.04em', textTransform: 'uppercase' },
  headerSub:   { fontFamily: F, fontSize: 13, color: 'var(--color-text-muted)', marginTop: 3 },
  // Section labels
  sectionLabel: { fontFamily: F, fontSize: 13, fontWeight: 700, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' },
  // Employee cards
  ...
  roleBadge: { fontFamily: F, fontSize: 13, padding: '3px 10px', background: 'rgba(96,48,255,0.2)', color: 'var(--color-accent-bright)', border: '1px solid rgba(96,48,255,0.45)', borderRadius: 4 },
  setFolderBtn: { fontFamily: F, fontSize: 13, color: 'var(--color-accent-bright)', cursor: 'pointer', background: 'none', border: 'none' },
  // Path row
  pathText:  { fontFamily: 'monospace', fontSize: 14, color: 'var(--color-text-muted)' },
  pathValue: { fontFamily: 'monospace', fontSize: 14, fontWeight: 700, color: 'var(--color-accent-bright)' },
  parentBtn: { fontFamily: F, fontSize: 13, padding: '4px 10px', cursor: 'pointer', borderRadius: 3 },
  // Subfolder chips
  folderChip: { fontFamily: F, fontSize: 14, padding: '5px 12px', cursor: 'pointer', borderRadius: 3 },
  // File rows
  fileName:  { fontFamily: 'monospace', fontSize: 14, fontWeight: 600, color: 'var(--color-text)' },
  fileSize:  { fontFamily: 'monospace', fontSize: 13, color: 'var(--color-text-muted)', whiteSpace: 'nowrap', marginLeft: 8 },
  // File section heading
  fileCount: { fontFamily: F, fontSize: 15, fontWeight: 700, color: 'var(--color-text)' },
  // Past deeds
  deedHeading: { fontFamily: F, fontSize: 15, fontWeight: 700, color: 'var(--color-accent-bright)' },
  ...
  deedPath:    { fontFamily: 'monospace', fontSize: 13, color: 'var(--color-text-muted)', lineHeight: 1.4 },
  deedSummary: { fontFamily: F, fontSize: 13, color: 'var(--color-text-muted)', lineHeight: 1.6, opacity: 0.85 },
  deedTime:    { fontFamily: 'monospace', fontSize: 12, color: 'var(--color-text-muted)', whiteSpace: 'nowrap' },
  // Tabs
  tabLabel: { writingMode: 'vertical-lr' as const, fontFamily: F, fontSize: 13, fontWeight: 700, color: 'var(--color-accent-bright)', textTransform: 'uppercase' as const, letterSpacing: '0.05em' },
  tabCount: { fontFamily: F, fontSize: 13, color: 'var(--color-text-muted)' },
  // Input
  customInput: { fontFamily: 'monospace', fontSize: 14, padding: '8px 12px', color: 'var(--color-text)', background: 'var(--color-bg-dark)', border: '1px solid var(--color-border)', outline: 'none', flex: 1 },
  customSetBtn: { fontFamily: F, fontSize: 14, fontWeight: 700, padding: '8px 14px', cursor: 'pointer', background: 'var(--color-accent)', color: '#fff', border: 'none', borderRadius: 2 },
  // Memory badge
  memoryBadge: { fontFamily: F, fontSize: 13, padding: '3px 9px', background: 'rgba(34,197,94,0.15)', color: 'rgb(134,239,172)', border: '1px solid rgba(34,197,94,0.35)', borderRadius: 4 },
...
export function EmployeeWorkspaceSidebar() {
  ...
  const fetchTree = async (subpath: string) => {
    try {
      const res = await fetch(`/api/workspace/tree?subpath=${encodeURIComponent(subpath)}`, { headers: authHeaders });
  ...
  useEffect(() => {
    ...
    const poll = setInterval(() => { void doRefresh(); }, 4000);
  ...
  const handleAssignFolder = async (folderPath: string) => {
    ...
    try {
      ...
      const res = await fetch('/api/employee/assign-folder', {
        method: 'POST',
        headers: authHeaders,
        body: JSON.stringify({ employeeName: activeWs.employeeName, folderPath: folderPath || '.' }),
      });
      if (res.ok) {
  ...
  const getFileIcon = (ext: string) => {
    const m: Record<string, string> = { '.ino': '⚡', '.py': '🐍', '.ts': '📘', '.tsx': '📘', '.js': '📜', '.jsx': '📜', '.css': '🎨', '.html': '🌐', '.json': '📋', '.md': '📝', '.cpp': '💻', '.c': '💻', '.rs': '💻' };
    return m[ext] ?? '📄';
  };
    ...
    <>
      ...
      {!isOpen && (
        ...
        >
          ...
          <span style={S.tabLabel} className="select-none">Workspaces</span>
          <span style={S.tabCount} className="select-none">({workspaces.length})</span>
        </button>
      )}
      ...
      <div
        className={`fixed top-0 left-0 bottom-0 z-40 flex flex-col bg-bg/98 backdrop-blur-md border-r-2 border-border shadow-2xl transition-all duration-200 ease-in-out pointer-events-auto ${
          isOpen ? 'w-[360px] translate-x-0' : 'w-0 -translate-x-full overflow-hidden pointer-events-none'
        }`}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b-2 border-border bg-bg-dark/90 select-none" style={{ padding: '12px 16px' }}>
          <div className="flex items-center" style={{ gap: 10 }}>
            ...
            <div>
              ...
              <div className="flex items-center" style={{ gap: 8, marginTop: 3 }}>
                ...
                <span
                  title={lastUpdated ? `Last updated: ${new Date(lastUpdated).toLocaleTimeString()}` : 'Auto-refresh active'}
                  style={{
                    display: 'inline-flex', alignItems: 'center', gap: 4,
                    fontFamily: F, fontSize: 11, fontWeight: 700,
                    color: 'rgb(134,239,172)', letterSpacing: '0.06em',
                    padding: '2px 6px',
                ...
                >
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'rgb(74,222,128)', display: 'inline-block', animation: 'pulse 2s infinite' }} />
                  LIVE
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center" style={{ gap: 8 }}>
            <button
              onClick={() => { fetchWorkspaces(); if (activeWs) fetchTree(activeWs.assignedFolder); }}
              disabled={isLoading}
              title="Refresh"
        ...
        {/* Feedback */}
        {feedback && (
          <div className="border-b border-accent bg-bg-dark" style={{ padding: '8px 16px', fontFamily: F, fontSize: 14, color: 'var(--color-accent-bright)' }}>
            {feedback}
          </div>
        )}

        {/* Employee selector */}
        <div className="border-b border-border/60 bg-bg-dark/50 flex flex-col" style={{ padding: '12px 14px', gap: 10 }}>
          <div className="flex items-center justify-between" style={S.sectionLabel}>
            <span>Team Members</span>
            <span style={{ fontWeight: 400, color: 'var(--color-accent-bright)', fontSize: 13 }}>click to select</span>
          </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="flex items-center truncate" style={{ gap: 6 }}>
                      ...
                      <span style={S.cardName} className="truncate">{ws.employeeName}</span>
                    </span>
                    {isSelected && <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--color-accent-bright)', flexShrink: 0 }} />}
                  </div>
                  <div className="flex items-center justify-between" style={{ gap: 4 }}>
                    <span style={S.cardPath} className="truncate">{ws.assignedFolder || 'root'}</span>
          ...
          <div className="flex-1 overflow-y-auto flex flex-col divide-y divide-border/60">
            ...
            <div className="bg-bg-dark/30 flex flex-col" style={{ padding: '14px 16px', gap: 12 }}>
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setIsAssigningCustom(!isAssigningCustom)} style={S.setFolderBtn}>
              ...
              {/* Custom path input */}
              {isAssigningCustom && (
                <div className="flex items-center bg-bg border border-accent rounded-sm" style={{ gap: 8, padding: 8 }}>
                  <input
                    type="text"
                  ...
                  />
                  <button
                    onClick={() => { if (customPathInput.trim()) { handleAssignFolder(customPathInput.trim()); setIsAssigningCustom(false); setCustomPathInput(''); } }}
              ...
              <div className="bg-bg-dark border border-border/80 rounded-sm flex flex-col" style={{ padding: '10px 12px', gap: 8 }}>
                <div className="flex items-center justify-between">
                  <span>
                    <span style={S.pathText}>📂 </span>
                    <span style={S.pathValue}>/{currentTree?.currentPath || activeWs.assignedFolder || 'root'}</span>
                  </span>
                ...
                {currentTree && currentTree.folders.length > 0 && (
                  <div className="flex flex-col pt-6 border-t border-border/40" style={{ gap: 8, paddingTop: 10 }}>
            ...
            <div className="flex flex-col" style={{ padding: '14px 16px', gap: 10 }}>
              <div className="flex items-center justify-between select-none">
                <span style={S.fileCount}>
                  📄 Files in Workspace
                  <span style={{ fontWeight: 400, fontSize: 13, color: 'var(--color-text-muted)', marginLeft: 6 }}>
              ...
              <div className="flex flex-col overflow-y-auto" style={{ gap: 5, maxHeight: 240 }}>
                {(!currentTree || currentTree.files.length === 0) && (
                  <div className="py-12 text-center border border-dashed border-border/40 rounded" style={S.empty}>
                ...
                {currentTree?.files.map((file) => (
                  ...
                  >
                    <div className="flex items-center truncate" style={{ gap: 8 }}>
                      ...
                      <span style={S.fileName} className="truncate">{file.name}</span>
                    </div>
            ...
            <div className="flex flex-col flex-1" style={{ padding: '14px 16px', gap: 10 }}>
              ...
              <div className="flex flex-col overflow-y-auto" style={{ gap: 8, flex: 1, maxHeight: 360 }}>
                {activeWs.pastDeeds.length === 0 && (
                  <div className="py-16 text-center border border-dashed border-border/40 rounded" style={S.empty}>
                ...
                {activeWs.pastDeeds.map((deed) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span style={S.deedFile} className="truncate">📄 {deed.fileName}</span>
                      <span style={S.deedTime}>{new Date(deed.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    <div style={S.deedTask}>Task: "{deed.task}"</div>
                    <div style={S.deedPath} className="truncate">📂 {deed.fullPath}</div>
